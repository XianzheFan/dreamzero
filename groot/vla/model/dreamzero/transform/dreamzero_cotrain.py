        ...
        if key == "text":
            ...
            for elem in features:
                ...
                try:
                    ...
                    elif elem["embodiment_id"] == embodiment_tag_mapping[EmbodimentTag.ROBOFACTORY.value]:
                        processed_item = "A multi-view video shows that two robot arms " + processed_item.lower() + " The video is split into four views: the top-left view shows the global scene camera, the top-right view shows the per-agent head camera, and the remaining views are black (inactive). The robot arms " + processed_item.lower()
                    else:
                    ...
                    if num_views > 1 and elem["embodiment_id"] == embodiment_tag_mapping[EmbodimentTag.AGIBOT.value]:
                        ...
                        item = "A multi-view video shows that two robot arms " + str(item).lower() + " The video is split into four views: the top-left view shows the global scene camera, the top-right view shows the per-agent head camera, and the remaining views are black (inactive). The robot arms " + str(item).lower()
                    else:
                        raise ValueError(f"Embodiment ID {elem['embodiment_id']} not supported.")
