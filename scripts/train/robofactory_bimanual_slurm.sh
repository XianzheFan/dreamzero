...
FOLLOWUP_TIME_LIMIT=${TIME_LIMIT:-}
if [ -z "${FOLLOWUP_TIME_LIMIT}" ] && [ -n "${SLURM_JOB_ID:-}" ]; then
    FOLLOWUP_TIME_LIMIT=$(scontrol show job "${SLURM_JOB_ID}" -o | tr ' ' '\n' | awk -F= '$1 == "TimeLimit" {print $2; exit}')
fi
FOLLOWUP_TIME_ARG=()
if [ -n "${FOLLOWUP_TIME_LIMIT}" ] && [ "${FOLLOWUP_TIME_LIMIT}" != "UNLIMITED" ]; then
    FOLLOWUP_TIME_ARG=(--time="${FOLLOWUP_TIME_LIMIT}")
fi
if ! NEXT=$(sbatch --parsable --dependency=afterany:${SLURM_JOB_ID} \
    --partition="${CURRENT_PARTITION}" \
    "${FOLLOWUP_TIME_ARG[@]}" \
    --export=ALL,TARGET_STEPS=${TARGET_STEPS},MAX_STEPS=${MAX_STEPS},BATCH_SIZE=${BATCH_SIZE},SAVE_STEPS=${SAVE_STEPS},SAVE_TOTAL_LIMIT=${SAVE_TOTAL_LIMIT},LEARNING_RATE=${LEARNING_RATE},REPORT_TO=${REPORT_TO},WANDB_PROJECT=${WANDB_PROJECT},WANDB_RUN_NAME=${WANDB_RUN_NAME},ACTION_LOSS_WEIGHT=${ACTION_LOSS_WEIGHT},GRIPPER_ACTION_LOSS_WEIGHT=${GRIPPER_ACTION_LOSS_WEIGHT},ACTION_PREFIX_LOSS_WEIGHT=${ACTION_PREFIX_LOSS_WEIGHT},ACTION_PREFIX_LOSS_LEN=${ACTION_PREFIX_LOSS_LEN},TIME_LIMIT=${FOLLOWUP_TIME_LIMIT},OUTPUT_DIR=${OUTPUT_DIR},ROBOFACTORY_DATA_ROOT=${ROBOFACTORY_DATA_ROOT},NUM_ARMS=${NUM_ARMS:-2},SHARED_GLOBAL=${SHARED_GLOBAL:-0} \
    "${REPO_DIR}/scripts/train/robofactory_bimanual_slurm.sh"); then
    echo "[$(date)] Failed to queue follow-up job on partition=${CURRENT_PARTITION} time_limit=${FOLLOWUP_TIME_LIMIT}"
    exit 1
fi
